...
`rmi-core-v25` tightens feature-credit evidence. Coverage that merely names an artist as a guest receives minimal weight and stays out of the main public news feed unless independent listening, chart, search, ticket, or corroborated public-reaction evidence confirms demand. Timely article candidates also receive most of the limited AI-verification slots. This revision does not repeat the v24 audience-scale rebase.

`rmi-core-v26` removes automatic valuation rebases from routine market runs. Long-term audience scale can only exert capped daily pressure during normal operation. A protected prelaunch reset may initialize all active artists directly at the same source-backed audience formula, zero daily changes, clear old charts and portfolios, and seed one new opening quote while preserving raw observations and verified news. The reset aborts before deleting anything if any active artist lacks enough direct audience evidence.
