...
- Do not rewrite recorded quote history to make a chart look better. Correct the universal model and preserve an auditable boundary between old and corrected model versions. Public charts apply only saved valuation factors to earlier observations at read time, retain `recordedPrice`, and offer Original quotes on the artist page. Never smooth, fabricate, or overwrite observations to hide a correction.
